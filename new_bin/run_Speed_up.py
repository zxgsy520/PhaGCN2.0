...
BIN = "/Work/pipeline/software/meta/PhaGCN2/v1.0.0/script/"
...
try:
    make_diamond_cmd = "diamond makedb --threads 8 --in {bin}/database/ALL_protein.fasta -d {bin}/database/database.dmnd".format(bin=BIN)
    print("Creating Diamond database...")
    _ = subprocess.check_call(make_diamond_cmd, shell=True)
    
    diamond_cmd = "diamond blastp --threads 8 --sensitive -d {bin}/database/database.dmnd -q {bin}/database/ALL_protein.fasta -o {bin}/database/database.self-diamond.tab".format(bin=BIN)
    print("Running Diamond...")
    _ = subprocess.check_call(diamond_cmd, shell=True)
    diamond_out_fp = "{bin}/database/database.self-diamond.tab".format(bin=BIN)
    database_abc_fp = "{bin}/database/database.self-diamond.tab.abc".format(bin=BIN)
...
for i in range(file_id):
    ...
    cmd = "python %s/run_KnowledgeGraph.py --n %s" % (BIN, str(i))
    try:
        out = subprocess.check_call(cmd, shell=True)
    except:
